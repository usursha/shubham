...
@RestController
@RequestMapping("/v2/portal/oauth")
@CrossOrigin("${app.cross-origin.allow}")
@Slf4j
public class PortalOAuthControllerResponse {

	private LoginService loginService;
	private TokenValidation tokenValidation;
	private RestUtilImpl restutil;

	public PortalOAuthControllerResponse(LoginService loginService, TokenValidation tokenValidation,
	...
	public ResponseEntity<ResponseDto<GenericDto>> loginUpdatePassword(
			@Valid @RequestBody LoginRequestDto loginRequestDto) {
		LoginResponseDto response = loginService.loginUpdatePassword(loginRequestDto);
		int statusCodeValue = HttpStatus.OK.value();
		String statusMessage = ResponseMessageUtil.getMessageForStatusCode(statusCodeValue);
		ResponseDto<GenericDto> formattedResponse = new ResponseDto<>();
		formattedResponse.setResponseCode(statusCodeValue);
	...
	public ResponseEntity<ResponseDto<ResponseMessageDto>> updatePasswordWithOTP(
		...
		ResponseDto<ResponseMessageDto> response = new ResponseDto<>();
		try {
			response.setData(loginService.changePasswordSplCharWithOutOTP(changePasswordRequest));
			response.setMessage("success");
			response.setResponseCode(HttpStatus.OK.value());
		} catch (Exception e) {
			response.setData(null);

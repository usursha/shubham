...
import com.hpy.ops360.ticketing.service.ManualTicketApproveOrRejectServiceByKafka;
import com.hpy.ops360.ticketing.service.RecentTicketPortalServiceHims;
...
public class TaskController {
	...
	@Autowired
	private ManualTicketApproveOrRejectServiceByKafka serviceByKafka;
	...
	@Autowired
	private RecentTicketPortalServiceHims recentTicketPortalServiceHims;
	...
	public ResponseEntity<ApproveRejectStatusresponseDto> processTicket_viaKafka(@PathVariable("status") String status,
			@RequestBody TaskApprovedTicketsDto taskDTO) {
//		ApproveRejectStatusDto approveRejectStatusDto = taskService.updateTicketsNumberData_ViaKafka(status, taskDTO);
		ApproveRejectStatusDto approveRejectStatusDto = taskService.updateTicketsNumberData_ViaKafkaHims(status, taskDTO);
		if (approveRejectStatusDto.getStatus().equals(SUCCESS)) {
			String result = String.format("%s %s Successfully", status, approveRejectStatusDto.getSynergyTicketNo());
			return new ResponseEntity<>(new ApproveRejectStatusresponseDto(true, result), HttpStatus.OK);
		} else {
	...
	@PostMapping("/action-kafka-extend/{status}")
	@Loggable
	public ResponseEntity<ApproveRejectStatusresponseDto> processTicket_viaKafka_extend(@PathVariable("status") String status,
			@RequestBody TaskApprovedTicketsDto taskDTO) {
		ApproveRejectStatusDto approveRejectStatusDto = null;
		try {
			approveRejectStatusDto = serviceByKafka.updateTicketsNumberData_ViaKafka(status, taskDTO);
		} catch (Exception e) {
...
//		return ResponseEntity.ok(taskService.getTicketDetailsByCEAndStatus_newResponse(ceId));
	...
	public ResponseEntity<TicketDataResponse> getTicketsByAtmId(@PathVariable String atmId, @PathVariable String ceId) {
		...
		return ResponseEntity
				.ok(taskService.transformToTicketDataResponse(recentTicketPortalServiceHims.getTicketDetailsByCEAgainstAtm(atmId, ceId)));
	}
...
//				request.getFromdate());
	...
	@Loggable
	public ResponseEntity<CeMachineUpDownCountDto> getCeMachineUpDownCount(@RequestBody UserDto userDto) {

		return ResponseEntity.ok(taskService.getCeMachineUpDownCount(userDto.getUsername()));
	}

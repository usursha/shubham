...
@Service
@Slf4j
public class AddEtaFromCmHimsAndSynergy {
	...
	public GenericResponseDto addEtaFromCmHimsAndSynergy(EtaDto2 etaDto, String token) {
		...
		try {
			...
			for (ParentChildTicketNo parentChildTicketNo : parentChildTicketList) {
				...
				if (!updateSuccess) {
					return new GenericResponseDto("Failure", "Failed to update ticket in " + systemSource + " system");
				}
			...
			log.info("ETA Updated Successfully for tickets By CM");
	...
	public GenericResponseDto addEtaFromCmHimsAndSynergyimage(EtaDto2 etaDto, String token) {
		log.info("EnhancedEtaService|addEta: {}", etaDto);
	...
	private boolean updateSynergyTicket(ParentChildTicketNo parentChildTicketNo, EtaDto2 etaDto) {
		try {
			...
			// Update subcall type
			SynergyResponse response = synergyService
					.updateSubcallType(UpdateSubcallTypeDto.builder().atmId(etaDto.getAtmid())
			...
			if (etaDto.getEtaDateTime() != null && !etaDto.getEtaDateTime().isEmpty()) {
				...
				UpdateEtaDto updateEtaDto = new UpdateEtaDto(parentChildTicketNo.getTicketid(), etaDto.getAtmid(),
						...
						etaDto.getCreatedBy(), "ETA updated by CE");
	...
	/**
	 * Validates and formats the ETA DateTime to the required format: dd/MM/yyyy
	 * HH:mm
	 * 
	...
	private String validateAndFormatEtaDateTime(String etaDateTime) {
		...
		DateTimeFormatter[] inputFormatters = { DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm"), // Already correct
																									// format
		...
		log.warn(
				"Unable to parse ETA DateTime: {}. Supported formats: dd/MM/yyyy HH:mm, dd/MM/yy HH:mm, yyyy-MM-dd HH:mm:ss, yyyy-MM-dd HH:mm, dd-MM-yyyy HH:mm, dd-MM-yy HH:mm, MM/dd/yyyy HH:mm, MM/dd/yy HH:mm, yyyy/MM/dd HH:mm",
				etaDateTime);
		return null;
	}
	...
	private boolean updateHimsTicket(ParentChildTicketNo parentChildTicketNo, EtaDto2 etaDto) {
		try {
			...
			HimsUpdateFollowUpDto himsUpdateDto = HimsUpdateFollowUpDto.builder()
					.ticketid(parentChildTicketNo.getTicketid()).atmid(etaDto.getAtmid())
					.etadatetime(etaDto.getEtaDateTime()).owner(etaDto.getOwner()).subcalltype(etaDto.getSubcallType())
					.customerRemark(etaDto.getCustomerRemark()).updatedby(loginService.getLoggedInUser())
					.comment(etaDto.getInternalRemark()).build();

			HimsResponse response = himsService.updateFollowUpDetails(himsUpdateDto);
			log.info("HIMS response: {}", response);

			if (response == null || !"Success".equals(response.getStatus())) {
				log.error("Failed to update HIMS ticket: {}", response != null ? response.getStatus() : "No response");
				return false;
			}
...
//		try {
//			etaRepository.addEta(etaDto.getAtmid(), etaDto.getCustomerRemark(),
//					etaDto.getEtaDocumentDto().getDocument(), etaDto.getEtaDocumentDto().getDocumentName(),
...
//					loginService.getLoggedInUser(), etaDto.getEtaDocumentDto().getDocument2(),
...
//	}
	private void saveToLocalRepository(ParentChildTicketNo parentChildTicketNo, EtaDto2 etaDto) {
		try {
			LocalDateTime parsedEtaDateTime = null;

			// Use CustomDateFormattor to parse the date and convert to LocalDateTime
			if (etaDto.getEtaDateTime() != null && !etaDto.getEtaDateTime().isEmpty()) {
				try {
					// First, validate the input can be parsed
					String validatedDateTime = CustomDateFormattor.convert(etaDto.getEtaDateTime(),
							CustomDateFormattor.FormatStyle.DATABASE);

					// Convert the DATABASE format string back to LocalDateTime
					parsedEtaDateTime = LocalDateTime.parse(validatedDateTime,
							DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));

					log.info("Converted ETA DateTime '{}' to LocalDateTime: '{}'", etaDto.getEtaDateTime(),
							parsedEtaDateTime);

				} catch (IllegalArgumentException e) {
					log.error("Failed to parse ETA DateTime: '{}'. Error: {}", etaDto.getEtaDateTime(), e.getMessage());
					parsedEtaDateTime = null;
				} catch (Exception e) {
					log.error("Error converting ETA DateTime to LocalDateTime: '{}'. Error: {}",
							etaDto.getEtaDateTime(), e.getMessage());
					parsedEtaDateTime = null;
				}
			} else {
				log.info("ETA DateTime is null or empty, skipping date formatting");
				parsedEtaDateTime = null;
			}

			etaRepository.addEta(etaDto.getAtmid(), etaDto.getCustomerRemark(),
					etaDto.getEtaDocumentDto().getDocument(), etaDto.getEtaDocumentDto().getDocumentName(),
					parsedEtaDateTime, // Pass LocalDateTime object instead of String
					etaDto.getInternalRemark(), etaDto.getOwner(), etaDto.getSubcallType(),
					parentChildTicketNo.getTicketid(), loginService.getLoggedInUser(),
					etaDto.getEtaDocumentDto().getDocument2(), etaDto.getEtaDocumentDto().getDocument1Name(),
					etaDto.getEtaDocumentDto().getDocument2(), etaDto.getEtaDocumentDto().getDocument2Name(),
					etaDto.getEtaDocumentDto().getDocument3(), etaDto.getEtaDocumentDto().getDocument3Name(),
					etaDto.getEtaDocumentDto().getDocument4(), etaDto.getEtaDocumentDto().getDocument4Name());

			log.info("Successfully saved ETA to local repository for ticket: {}", parentChildTicketNo.getTicketid());

		} catch (Exception e) {
			log.error("Error saving to local repository for ticket: {}", parentChildTicketNo.getTicketid(), e);
			// Consider whether to throw the exception or handle it gracefully
			throw new RuntimeException("Failed to save ETA to local repository", e);
		}
	}

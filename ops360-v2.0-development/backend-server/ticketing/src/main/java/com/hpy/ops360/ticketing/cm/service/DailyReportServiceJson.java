...
@Service
@Slf4j
public class DailyReportServiceJson {
	...
	public List<ReportDetailsDto> generateReport(String calltype, int isbreakdown, String OpenDate_uid,
			String status, int pageindex, int dataperpage) {


		List<ReportDetailsEntity> reportPage = reportRepository.getOpsDailyReportData(calltype, isbreakdown, OpenDate_uid, status, pageindex, dataperpage);


		// Convert the Page to a List of DTOs
		List<ReportDetailsDto> response = reportPage.stream().map(result -> new ReportDetailsDto(result.getSr_no(),
				result.getTicketNumber(), result.getBankAsPerSynergy(), result.getBankAsPerOps360(), result.getAtmId(),
				result.getAtmCategory(), result.getSiteType(), result.getAddress(), result.getCity(), result.getState(),
				result.getZone(), result.getZonalHead(), result.getStateHead(), result.getChannelManager(),
				result.getChannelExecutive(), result.getFieldServiceCoordinator(), result.getOwner(),
				result.getSubCall(), result.getEventCode(), result.getVendor(), result.getDowntimeHoursMin(),
				result.getDowntimeBucket(), result.getTicketCreatedDateTime(), result.getExpectedTat(),
				result.getFirstDispatchDateTime(), result.getReallocatedDocketNo(), result.getLastAllocatedBy(),
				result.getLastAllocatedDateTime(), result.getCesActionDateTime(), result.getCeEtaUpdatedDateTime(),
				result.getCallCloseDateTime(), result.getCallCloseWithinTatOrOutOfTat(), result.getCustomerRemarks(),
				result.getCeInternalRemarks(), result.getLastActivityDateTime(), result.getLastCommentAsPerSynergy(),
				result.getNextFollowUpDateTime(), result.getActionTaken(), result.getTicketStatus())).toList();

		return response;
	}
	
	
	public List<CEWiseUptimeReportDto> viewCEWiseuptimeReport(String rcmUserId, String scmUserId, String cmUserId, String ceUserId, String startDate, String endDate,  int page, int pageSize) {
    	...
    	List<CEWiseUptimeReportEntity> reportData = ceWiseReportRepository.getCeWiseviewData(rcmUserId, scmUserId, cmUserId, ceUserId, startDate, endDate, page, pageSize);
...
//        		result.getSrno(),
	...
	public List<AtmIdUptimeReportDto> viewATMIdWiseuptimeReport(String startDate, String endDate, String atmIds, String status, int page, int pagesize) {
        log.info("**** Inside downloadCeWiseUptimeReport Service ****");
        log.info("Request received: startDate=" + startDate + ", endDate=" + endDate + ", atmIds=" + atmIds + ", status=" + status);
    	List<AtmIdUptimeReportEntity> reportData = atmIdWiseReportRepository.getAtmIdWiseviewData(startDate, endDate, atmIds, status, page, pagesize);
        log.info("**** Inside generateCEWiseuptimeReport ****");
        log.info("Report data Returned from repository: "+ reportData);
